...
import { AIManager, type AIConfig, type AIAnalysisRequest } from '../ai/ai.manager.js';
...
export class MCPServer {
  ...
  constructor(aiConfig?: AIConfig) {
    ...
    // Initialize AI manager if config provided
    if (aiConfig) {
      this.initializeAI(aiConfig);
    }
  }

  private async initializeAI(aiConfig: AIConfig): Promise<void> {
    try {
      this.aiManager = new AIManager(aiConfig);
      await this.aiManager.initialize();
      console.log('🤖 AI Manager integrated with MCP Server');
    } catch (error) {
      console.warn('Failed to initialize AI Manager:', error);
...
*Note: This is a mock response. In a real implementation, the code would be executed in the browser context with proper sandboxing.*`
    ...
    try {
      ...
      if (this.aiManager?.isReady()) {
        const aiRequest: AIAnalysisRequest = {
          ...
          includeSuggestions,
          includeCodeExamples,
          context: `User: ${this.context.userId}, Session: ${this.context.sessionId}`
        };

        const aiResponse = await this.aiManager.analyzeElement(aiRequest);
        
        let analysis = `# AI Analysis: ${element.tagName} Element\n\n`;
        analysis += `**Source:** ${aiResponse.source} | **Confidence:** ${(aiResponse.confidence * 100).toFixed(1)}%\n\n`;
        ...
        if (aiResponse.codeExamples.length > 0) {
          analysis += `\n\n## Code Examples\n`;
          aiResponse.codeExamples.forEach((example, index) => {
            analysis += `\n### Example ${index + 1}\n`;
            analysis += `\`\`\`\n${example}\n\`\`\`\n`;
          });
        }

        analysis += `\n\n---\n*Analysis generated by ${aiResponse.metadata.model} in ${aiResponse.metadata.processingTime}ms*`;
        ...
        let analysis = `# Basic Analysis: ${element.tagName} Element\n\n`;
        analysis += `**Note:** AI services are not available. Using basic analysis.\n\n`;
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Error during AI analysis: ${error instanceof Error ? error.message : 'Unknown error'}`
          }
        ],
        isError: true
      };
  ...
  private async handleGenerateTestCode(args: any): Promise<CallToolResult> {
    ...
    try {
      ...
      // Use AI manager if available
      if (this.aiManager?.isReady()) {
        testCode = await this.aiManager.generateTestCode(element, testType, framework);
      } else {
        // Fallback to basic test code generation
        testCode = this.generateTestCode(element, testType, framework, includeSetup);
      }
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Error generating test code: ${error instanceof Error ? error.message : 'Unknown error'}`
          }
        ],
        isError: true
      };
  ...
  private async handleGetCursorContext(args: any): Promise<CallToolResult> {
    ...
    const _maxContextLines = args.maxContextLines || 50;
